...
export const ModeSelector: React.FC<ModeSelectorProps> = ({ currentMode, setMode, disabled = false }) => {
    return (
        <div className={`mb-8 p-4 bg-gray-800/50 border border-gray-700 rounded-lg transition-opacity ${disabled ? 'opacity-50' : ''}`}>
            <h2 className="text-lg font-semibold text-gray-200 mb-3 text-center sm:text-left">Select Generation Mode</h2>
            <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                {Object.values(GenerationMode).map(mode => (
                    <button 
                        ...
                        disabled={disabled}
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-gray-900
                            ${currentMode === mode 
            ...
            <p className="text-center sm:text-left mt-3 text-sm text-gray-400 min-h-[2.5rem] px-1">
